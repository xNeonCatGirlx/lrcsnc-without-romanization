...
// InitialModel initializes and returns a new model for TUI with default settings.
// It sets up the help model with specific styles for key and description,
// and initializes various fields of the model struct.
// The function also initializes a progress bar with a specified color.
func InitialModel() model {
	...
	return model{
		...
		progressBar: progress.New(progress.WithSolidFill(config.Theme.ProgressBarColor)),
		help:        helpModel,
	}
}
...
func (m model) progressBarTick() tea.Cmd {
	return tea.Tick(time.Duration(1*player.Rate*1000)*time.Millisecond, func(_ time.Time) tea.Msg { return animateProgressBarTick(true) })
}
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case configChanged:
		reloadStyles()
		m.progressBar.FullColor = config.Theme.ProgressBarColor
		return m, watchConfigChanges()
	case songInfoChanged:
		if player.Song.LyricsData.LyricsType >= 2 {
			m.lyricLines = [][]string{{"───"}}
		} else {
			m.lyricLines = make([][]string, 0, len(player.Song.LyricsData.Lyrics))
			for _, s := range player.Song.LyricsData.Lyrics {
				m.lyricLines = append(m.lyricLines, m.lyricWrap(s.Text))
			}
		}
		...
		if player.Song.LyricsData.LyricsType != 0 {
			m.followSync = false
		} else {
			m.followSync = true
		}
	...
	case playerInfoChanged:
		return m, tea.Batch(watchPlayerInfoChanges(), m.progressBar.SetPercent(math.Round(player.Position)/math.Round(player.Song.Duration)))

	case animateProgressBarTick:
		if player.PlaybackStatus == mprislib.PlaybackPlaying {
			player.Position++
		}
	...
	case overwriteReceived:
		...
		return m, tea.Batch(watchReceivedOverwrites(), tea.Tick(5*time.Second, func(_ time.Time) tea.Msg { return overwriteReceived("") }))
	...
	case tea.KeyMsg:
		...
		case "enter":
			if player.Song.LyricsData.LyricsType != 0 || m.currentLyric == -1 {
				return m, nil
			}

			if err := mpris.SetPosition(player.Song.LyricsData.Lyrics[m.cursor].Time); err != nil {
...
func (m model) headerView() string {
	...
	if m.overwrite != "" {
		...
		title = gloss.NewStyle().Foreground(gloss.Color("15")).AlignHorizontal(gloss.Center).Render(fmt.Sprintf("%s - %s%s", strings.Join(player.Song.Artists, ", "), player.Song.Title, m.header))
	}
...
func (m model) footerView() string {
	...
	position := gloss.NewStyle().AlignHorizontal(gloss.Left).Render(positionIntoString(int(math.Round(player.Position))) + " ")
	duration := gloss.NewStyle().AlignHorizontal(gloss.Right).Render(" " + positionIntoString(int(math.Round(player.Song.Duration))))
	m.progressBar.Empty = ' '
	m.progressBar.Full = '█'
	m.progressBar.Width = max(0, m.w-gloss.Width(position)-gloss.Width(duration)-4)
	m.progressBar.ShowPercentage = false
	return gloss.NewStyle().BorderTop(true).BorderStyle(gloss.NormalBorder()).Margin(0, 2).Render(gloss.JoinHorizontal(gloss.Center, position, m.progressBar.ViewAs(math.Round(player.Position)/math.Round(player.Song.Duration)), duration))
}
...
func (m model) lyricsView() string {
	...
	case types.LyricsStateNotFound:
		return "This song was not found!"
	case types.LyricsStateInProgress:
		return "Loading lyrics..."
	case types.LyricsStateUnknown:
		return ":c"
		...
		for i, lyricLine := range m.lyricLines {
			...
			if i < m.currentLyric {
				for _, l := range lyricLine {
					stylizedLyrics = append(stylizedLyrics, styleLyricBefore.Render(l))
				}
				line = gloss.JoinVertical(gloss.Center, stylizedLyrics...)
			} else if i == m.currentLyric {
				for _, l := range lyricLine {
					stylizedLyrics = append(stylizedLyrics, styleLyricCurrent.Render(l))
				}
				line = gloss.NewStyle().Margin(1, 0).Render(gloss.JoinVertical(gloss.Center, stylizedLyrics...))
			} else if i > m.currentLyric {
				for _, l := range lyricLine {
					stylizedLyrics = append(stylizedLyrics, styleLyricAfter.Render(l))
				}
				line = gloss.JoinVertical(gloss.Center, stylizedLyrics...)
			}
			...
			if !m.followSync {
				if i == m.cursor {
					stylizedLyrics = make([]string, 0, len(lyricLine))
					if i == m.currentLyric && player.Song.LyricsData.LyricsType == 0 {
						for _, l := range lyricLine {
							stylizedLyrics = append(stylizedLyrics, styleLyricCurrent.Render(l))
						}
						line = gloss.JoinVertical(gloss.Center, stylizedLyrics...)
						...
						for _, l := range lyricLine {
							stylizedLyrics = append(stylizedLyrics, styleLyricCursor.Render(l))
						}
						line = gloss.JoinVertical(gloss.Center, stylizedLyrics...)
					}
				}
			...
			if m.showTimestamps {
				style := styleTimestampBefore
				if i == m.cursor && i == m.currentLyric {
					style = styleTimestampCurrent
				} else if i == m.cursor {
					style = styleTimestampCursor
				} else if i > m.currentLyric {
					style = styleTimestampAfter
				}
				timestampView = style.Render(timestampIntoString(player.Song.LyricsData.Lyrics[i].Time)) + " "
			}

			line = gloss.JoinHorizontal(gloss.Center, timestampView, line)

			if i == m.currentLyric && i == m.cursor && !m.followSync && player.Song.LyricsData.LyricsType == 0 {
...
func (m model) lyricWrap(l string) (res []string) {
	if gloss.Width(styleLyricCurrent.Render(l)) < m.lyricViewport.Width*3/4 {
		return []string{l}
	}
	...
	for i := 0; i < len(words); i++ {
		if gloss.Width(styleLyricCurrent.Render(s.String()+" "+words[i])) >= m.lyricViewport.Width*2/3 {
